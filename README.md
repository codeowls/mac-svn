...
- 打开已有工作副本，保存最近打开记录；点击记录右侧的 × 可从列表移除，重启后仍生效，不删除磁盘文件，也不关闭当前工作副本。从子目录打开时定位到工作副本根目录。
...
测试使用 `svnadmin` 创建隔离的本地 `file://` 仓库，覆盖两个工作副本之间的检出、更新、选择性提交、目录属性、中文/空格/@/前导连字符文件名、冲突拒绝、XML 解析和子进程取消；另覆盖分支目录浏览、中文及 @ 分支 URL、仅检出选中分支、直接检出到已有空文件夹、拒绝非空目录和普通文件目标、多层目录及空目录的递归检出、忽略 externals、无效 URL 和远端目录错误。测试不访问业务仓库，产生的临时目录以 `mac-svn-tests-` 开头，为便于排障保留，不自动删除。
